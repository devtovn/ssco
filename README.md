...
| Biến                               | Gợi ý                                                                                      |
| ---------------------------------- | ------------------------------------------------------------------------------------------ |
| `POSTGRES_PASSWORD`                | Mật khẩu Postgres (mặc định: `pricecompare_dev_password`) — **ghi nhớ** cho bước migration |
...
$env:DATABASE_URL = "postgresql://pricecompare:YOUR_PASSWORD@localhost:5432/price_comparison"
npm run migrate:up
```
...
Phải có user `admin@pricecompare.vn` | `Administrator`.
...
Đổi `pricecompare_dev_password` nếu bạn đã đổi `POSTGRES_PASSWORD` trong `.env`.
