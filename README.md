...
Spec & kế hoạch triển khai: [`.kiro/specs/product-price-comparison-website/`](.kiro/specs/product-price-comparison-website/).
...
├── packages/types/   # Shared TypeScript types (@price-comparison/types)
├── docker-compose.yml
...
Migration **không** tự chạy khi container lên. Chạy từ máy host (cần Node.js):
...
Sửa `backend\.env` — đặt `DATABASE_URL` trùng với Docker Postgres (mặc định từ `.env` root):

```env
DATABASE_URL=postgresql://pricecompare:pricecompare_dev_password@localhost:5432/price_comparison

DB_HOST=localhost
DB_PORT=5432
DB_NAME=price_comparison
DB_USER=pricecompare
DB_PASSWORD=pricecompare_dev_password
...
API chạy tại **http://localhost:4000** (đặt `PORT=4000` trong `backend\.env`).
...
**`@price-comparison/types` not found**
...
| [`.kiro/specs/.../requirements.md`](.kiro/specs/product-price-comparison-website/requirements.md) | Yêu cầu nghiệp vụ |
| [`.kiro/specs/.../tasks.md`](.kiro/specs/product-price-comparison-website/tasks.md) | Kế hoạch task |
